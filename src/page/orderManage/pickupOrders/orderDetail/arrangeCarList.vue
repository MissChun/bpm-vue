  ...
  methods: {
    ...
    clicktabs: function(targetName) {
      if (targetName.name == 'second') {
        this.$router.push({ path: `/orderManage/pickupOrders/orderDetail/arrangeCarTab/arrangeCarMap/${this.id}` });
      }
    },
        ...
        if (results.data && results.data.code == 0 && results.data.data) {
          ...
          if (needJudge) {
            vm.$confirm("当前需确认车辆已经发生改变,请重新确认", '提示', {
              ...
              vm.$router.go(0);
              //vm.$router.push({ path: `/orderManage/pickupOrders/orderDetail/arrangeCarTab/arrangeCarList/${vm.id}/${vm.operationStatus}` });
            });
          } else {

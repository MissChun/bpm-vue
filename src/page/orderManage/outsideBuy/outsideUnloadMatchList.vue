          ...
          <el-tab-pane label="列表" name="first">
            <div class="tab-screen">
              <el-header>
                <el-row>
                  <el-col :span="3">
                    <router-link :to="{path: '/orderManage/outsideBuy/outsideBuyList'}">
...
export default {
  ...
  methods: {
    ...
    operation: function(type, row) {
      ...
      if (type == 'sureMatch') {
        ...
        vm.$$http("upMatchUnloadList", sendData).then(results => {
          ...
          vm.$router.push({ path: "/orderManage/outsideBuy/outsideBuyList" });
        });
      }
    },
    getSearchLIst: function() {
      ...
      sendData.out_buy_waybill_id = this.id;
    ...
    getList: function() {
      ...
      this.$$http("getMatchBusiness", sendData1).then((results) => {
        vm.needNum++;
        if (results.data.code == 0) {

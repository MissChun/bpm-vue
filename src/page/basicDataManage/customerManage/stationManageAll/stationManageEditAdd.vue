  ...
  <div class="out-contain">
    <el-header>
      <el-row>
        <el-col :span="3">
          <router-link
            :to="{path: '/basicDataManage/customerManage/stationManageAll/stationManageList'}"
          >
        ...
        <el-col :span="4">
          <el-form-item>
            <el-button
              type="primary"
              @click="startSearch"
              :loading="searchBtn.loading"
              :disabled="searchBtn.isDisabled"
            >{{searchBtn.text}}</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="实际站点:" prop="map_position">
                <div class="no-wrap">{{formData.map_position}}</div>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="20">
              <el-form-item label="站点类型:" prop="station_type">
                <el-select v-model="formData.station_type" placeholder="请选择">
                  <el-option
                    v-for="(item,key) in stationTypeSelect"
                    :key="key"
                    :label="item.verbose"
                    :value="item.key"
                  ></el-option>
            ...
            <el-col :span="20">
              <el-form-item label="站点类别:">
                <el-select v-model="formData.station_category" placeholder="请选择">
                  <el-option
                    v-for="(item,key) in stationCategoryList"
                    :key="key"
                    :label="item.label"
                    :value="item.value"
                  ></el-option>
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="地址:">
                <div class="no-wrap">{{formData.address}}</div>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
    ...
    <transition name="fade">
      <div
        class="side-alert-traggle side-alert-traggle-right"
        v-show="showLeftWindow"
        @click="showLeftWindow = !showLeftWindow"
      >
        <span>收起</span>
      </div>
    </transition>
    <transition name="fade">
      <div
        class="side-alert-traggle side-alert-traggle-left"
        v-show="!showLeftWindow"
        @click="showLeftWindow = !showLeftWindow"
      >
        <span>展开</span>
    ...
    return {
      ...
      rules: {
        position_name: [
          ...
          {
            pattern: /^([\u4E00-\u9FA5A-Za-z0-9]{0,10})$/gi,
            message: "地标名称为1～10个字符",
            trigger: "blur"
          }
        ],
        map_position: [
  ...
  methods: {
        ...
        (MarkerList, SimpleMarker, SimpleInfoWindow, BasicControl) => {
          ...
          this.markerList = new MarkerList({
            ...
            getInfoWindow: function(data, context, recycledInfoWindow) {
              let infoTitleStr =
                '<div class="marker-info-window"><span class="fs-13">' +
                data.position_name +
                "</span>";
              let infoBodyStr =
                '<br><div class="fs-13 text-center">数据加载中...</div><br>';
          ...
          this.markerList.on("selectedChanged", (event, info) => {
            if (info.selected) {
              ...
              this.getLandmarkDetail(id)
                ...
                .catch(() => {
                  let infoBodyStr =
                    '<br><div class="fs-13 text-center">数据加载失败</div><br>';
                  infoWindow.setInfoBody(infoBodyStr);
                });
            }
          });
    ...
    getCustomerList: function() {
      return new Promise((resolve, reject) => {
        ...
        this.$$http("searchPayCustomerList", postData)
          .then(results => {
    ...
    getActualSiteList: function() {
      return new Promise((resolve, reject) => {
        ...
        if (
          this.searchFilters.keyword.length &&
          this.searchFilters.field === "position_name"
        ) {
            ...
            if (this.siteOfCusmerDetail.map_position) {
              this.getLandmarkDetail(this.siteOfCusmerDetail.map_position).then(
                () => {
                  ...
                  this.map.setZoom(15);
                  this.map.setCenter([
                    this.landmarkDetail.location.longitude,
                    this.landmarkDetail.location.latitude
                  ]);
    ...
    startSearch: function() {
      ...
      this.getActualSiteList().then(data => {
        //展示该数据
        this.renderMarker();
    ...
    addEditStationAjax: function(apiName) {
      return new Promise((resolve, reject) => {
        let area =
          this.choosedActualSite.county &&
          this.choosedActualSite.county.area_name
            ? this.choosedActualSite.county.area_name
            : "";
        let city =
          this.choosedActualSite.city && this.choosedActualSite.city.area_name
            ? this.choosedActualSite.city.area_name
            : "";

        let province =
          this.choosedActualSite.province &&
          this.choosedActualSite.province.area_name
            ? this.choosedActualSite.province.area_name
            : "";
        ...
        this.$$http(apiName, postData)
          .then(results => {
            if (results.data && results.data.code == 0) {
              ...
              this.$router.push({
                path:
                  "/basicDataManage/customerManage/stationManageAll/stationManageList"
              });
    ...
    getInfoWindowDom: function(data) {
      let source_type =
        data.source_type && data.source_type.verbose
          ? data.source_type.verbose
          : "无";
      let infoBodyStr =
        '<div class="fs-13 md-5">地标位置：' +
        data.address +
        '</div><div class="fs-13">上传来源：' +
        source_type +
    ...
    getLandmarkDetail: function(id) {
      return new Promise((resolve, reject) => {
        ...
        this.$$http("getLandMarkDetail", postData)
          .then(results => {
            this.pageLoading = false;
            if (results.data && results.data.code == 0) {
    ...
    getSiteOfCustomerDetail: function() {
      return new Promise((resolve, reject) => {
        ...
        this.$$http("getSiteOfCustomerDetail", postData)
          .then(results => {
            if (results.data && results.data.code == 0) {
              ...
              this.formData.short_name =
                (this.siteOfCusmerDetail.owner &&
                  this.siteOfCusmerDetail.owner.length &&
                  this.siteOfCusmerDetail.owner[0]) ||
                "";

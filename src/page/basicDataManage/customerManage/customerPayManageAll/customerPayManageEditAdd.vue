      ...
      <el-main v-loading="pageLoading">
        <transition name="el-fade-in-linear">
          <div v-if="activeStep==0">
            <el-form class="addTailcarform" label-width="120px" ref="addEditFormSetp1" :rules="rules" :model="customerPayFrom" status-icon>
              <el-row :gutter="40">
                <el-col :span="8">
                ...
                <el-col :span="8" >
                  <el-form-item label="客户名称:" prop="consumer" >
                    <el-select v-model="customerPayFrom.consumer" multiple  filterable placeholder="请选择客户" v-loading="customerLoading">
  ...
  methods: {
    ...
    goDetalis: function() {
      if (this.$route.query.operate === 'edit') {
        this.$router.push({
          path: `/basicDataManage/customerManage/customerPayManageAll/customerPayManageDetalis/${ this.customerPayId }/`
        });
      } else {
          ...
          this.$$http('upadtecustomerPayFrom', sendData).then((result) => {
            if (result.data.code == 0) {
              ...
              if (operation == 'out') {
                ...
                vm.$router.push({
                  path: "/basicDataManage/customerManage/customerPayManageAll/customerPayManageEditAdd",
                  query: {

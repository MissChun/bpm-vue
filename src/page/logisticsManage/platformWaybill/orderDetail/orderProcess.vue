  ...
  methods: {
    ...
    sureChangeCar: function(rowData) {
      ...
      this.$$http('sureChangeCar', sendData).then(results => {
        ...
        this.$router.push({ path: "/logisticsManage/platformWaybill/ordersList" });
        if (results.data.code == 0) {
          vm.$message({
            type: "success",
            message: "确认变更成功"
    ...
    clicktabs: function(targetName) {
      if (targetName.name == 'first') {
        this.$router.push({ path: `/logisticsManage/platformWaybill/orderDetail/orderDetailTab/${this.setpId}/${this.willId}` });
      }

          ...
          <el-row :gutter="10">
            <el-col :span="12">
              <div class="label-list">
                <label>供应商:</label>
                <div class="detail-form-item">{{row.supplier}}</div>
              </div>
            </el-col>
            <el-col :span="12">
              <div class="label-list">
                <label>装车完成时间:</label>
                <div class="detail-form-item">{{row.work_end_time}}</div>
              </div>
            </el-col>
          </el-row>
          <el-row :gutter="10">
            <el-col :span="12">
              <div class="label-list">
                <label>供方液厂名称:</label>
                <div class="detail-form-item">{{row.fluid}}</div>
              </div>
            </el-col>
            <el-col :span="12">
              <div class="label-list">
                <label>实际装车吨位:</label>
                <div class="detail-form-item">{{row.active_tonnage}}</div>
              </div>
            </el-col>
          </el-row>
          <el-row :gutter="10">
	          ...
	          <el-row style="min-height: 131px;">
	            <el-col :span="5" :offset="1" v-if="!poundUpload.fileList.length&&loadImgArr.length">
...
export default {
  ...
  methods: {
    	...
    	this.$$http('getConOrderDetail', postData).then((results) => {
	        if (results.data && results.data.code == 0 && results.data.data) {
	          ...
	          for (var i = 0; i < this.detailData.trips.length; i++) {
	            if (this.detailData.trips[i].section_type.key == 'pickup') {
	...
	forExportDetail(type){
    	...
    	this.imgLoading = true;
    	if(type === 'forExport'){
    		apiName = 'outsidePickDetalis';
    	}else{
    		apiName = 'outsideBuyDetalis';
    	}
    	this.$$http(apiName, postData).then((results) => {
	        ...
	        if (results.data && results.data.code == 0 && results.data.data) {
	         ...
	         // console.log('data',results.data.data);
	          /* 获取运力 */
	          if(results.data.data.weight_note_image_url){
	          	  this.loadImgArr = [{
		        	id:results.data.data.id,
		        	url:results.data.data.weight_note_image_url
		          }];
			      this.imgReviewSrcUrl = this.imgReviewSrc();
	          }
	...
	updateWaybill(){
		...
		this.$$http('updatePurchaseWaybillStatus', postData).then((results) => {
	        if (results.data && results.data.code == 0 && results.data.data) {
	...
	updateBillboardImg(type){
		...
		let postData = {};
		if(type==='forExport'){
			apiName='updateForExportImg';
		    postData = {
		    	id:this.detailData.id,
		    	weight_note_image_url:this.poundUpload.fileList[0].url
		    };
		}else if(type==='externalMining'){
			apiName = 'updateExternalMiningImg';
			postData = {
		    	id:this.detailData.id,
		    	weight_note_image_url:this.poundUpload.fileList[0].url
		    };
	...
	updateBtn(){
		let type = '';
		if (this.row.waybill.indexOf("TSE") != -1) {
      		type = 'forExport';
        }else if(this.row.waybill.indexOf("TE") != -1){
        	type = 'externalMining';
        }
  ...
  watch: {
    isShow(curVal, oldVal) {　
      if(curVal){
      	if (this.row.waybill.indexOf("TSE") != -1) {
      		this.forExportDetail('forExport');
        } else if (this.row.waybill.indexOf("TE") != -1) {
      		this.forExportDetail();
        } else {
          this.getDateDetail();
        }
